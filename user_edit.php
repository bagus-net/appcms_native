...
if(isset($_POST['btnSimpan'])){		
	...
	if (count($pesanError)>=1 ){
		...
		if (trim($txtPassword)=="") {
			...
			$passwSQL = ", pass='$txtPassLama'";
		}
		else {
//			$passwSQL = ",  pass ='".md5($txtPassword)."'";
			$passwSQL = ",  pass ='$txtPassword'";
		}
		...
		$mySql  = "UPDATE users SET keterangan='$txtNama', level='$txtUsername'
					$passwSQL
					WHERE nama='$Kode'";
					
		$myQry=pg_query($koneksidb, $mySql) or die ("Gagal query coy $mySql ".pg_last_error());
		if($myQry){
...
$Kode	= $_GET['Kode']; 
$mySql	= "SELECT * FROM users WHERE nama='$Kode'";
$myQry	= pg_query($koneksidb, $mySql)  or die ("Query ambil data salah : ".pg_last_error());
$myData = pg_fetch_array($myQry);

	// Data Variabel Temporary (sementara)
	$dataKode		= $myData['nama'];
	$dataNama		= isset($_POST['txtNama']) ? $_POST['txtNama'] : $myData['keterangan'];
	$dataUsername	= isset($_POST['txtUsername']) ? $_POST['txtUsername'] : $myData['level']; 
...
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self" id="form1">
  <table class="table-list" width="650" border="0" cellspacing="1" cellpadding="3">
    ...
    <tr>
      ...
      <input name="txtPassLama" type="hidden" value="<?php echo $myData['pass']; ?>" /></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
